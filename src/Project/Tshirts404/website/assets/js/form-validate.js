function valid_datas(f) {
  if (f.name.value == "") {
    ...
    jQuery.ajax({
      url: "mail.php",
      type: "post",
      data: jQuery("form#404ShirtSSC-contact").serialize(),
      complete: function (data) {
        jQuery("#form_status").html(data.responseText);
        jQuery("#404ShirtSSC-contact")
          .find("input,textarea")
          .attr({ value: "" });
        jQuery("#404ShirtSSC-contact").css({ opacity: 1 });
        jQuery("#404ShirtSSC-contact").remove();
      },
    });
    jQuery("#form_status").html(
      '<span class="loading">Sending your message...</span>'
    );
    jQuery("#404ShirtSSC-contact").animate({ opacity: 0.3 });
    jQuery("#404ShirtSSC-contact")
...
function notice(f) {
  jQuery("#404ShirtSSC-contact").find("input,textarea").css("border", "none");
